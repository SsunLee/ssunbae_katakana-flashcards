...
import PageControls from "@/app/components/controls/PageControls";
...
export default function JapaneseVerbsPage() {
  ...
  const [expanded, setExpanded] = useState(false); // ✅ 페이지가 확장 상태 제어
  ...
  useEffect(() => {
    ...
    if (flipped) {
      t = setTimeout(() => setShowForms(true), 220); // ✅ 딜레이 (원하면 250~300 조정)
    } else {
      setShowForms(false);
      setExpanded(false); // 앞면으로 돌아가면 접힘으로 초기화(선택)
    }
    return () => { if (t) clearTimeout(t); };
  }, [flipped]);


  // 그리드 전용: 카드별 접힘/펼침 + 플립 상태
  const [gridExpandedStates, setGridExpandedStates] = useState<Record<number, boolean>>({});
  ...
  const toggleGridExpand = (id: number) =>
    setGridExpandedStates((prev) => ({ ...prev, [id]: !prev[id] }));
  ...
  const speakVerb = (v: Verb) => {
    if (!ttsReady || !isTtsSupported) return;
    speakJa(v.reading || v.kanji);
  };
      ...
      {viewMode === "single" && (
        <div className="mb-4 flex w-full max-w-md items-center justify-between text-sm mx-auto">
          ...
          <SettingsDialog
            ...
            resetDeck={reset}
            sentenceFontSize={verbFontSize}
            setSentenceFontSize={setVerbFontSize}            
          />
        </div>
      )}
          ...
          current && (
            <>
              <VerbCardView
                ...
                titleFontSize={verbFontSize}  // SettingsDialog 슬라이더 값
              />
              {/* ✅ 전면(앞면)일 때는 표를 가리고, 뒷면일 때만 표를 노출 */}
              {showForms && (
                <VerbFormsTable
                  verb={current}
                  expanded={expanded}
